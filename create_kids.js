...
const formsDir = path.join(__dirname, 'client', 'src', 'pages', 'admin');

// 1. Model
const kidContentModel = `const mongoose = require('mongoose');
...
fs.writeFileSync(path.join(modelsDir, 'KidContent.js'), kidContentModel);

// 2. Route
const kidContentRoute = `const express = require('express');
...
router.get('/', async (req, res) => {
  try {
    const items = await KidContent.find().sort({ orderIndex: 1, createdAt: -1 });
    res.json(items);
  } catch (error) {
...
fs.writeFileSync(path.join(routesDir, 'kidContent.js'), kidContentRoute);

// 3. Admin Form
const adminForm = `import React, { useState, useEffect } from 'react';
...
fs.writeFileSync(path.join(formsDir, 'AdminKidContentForm.jsx'), adminForm);

console.log("Done generating files for Kids!");
